...
기능은 아직 구현 전이다. 아래 인증 헤더는 현재 코드의 계약이며, 팀의 최종 헤더
통일·키 배포 절차와 오프라인 정책 확인은 별도로 필요하다.
...
- 두 GET 모두 **등록된 ACTIVE 키오스크의 API Key**가 필요하다.
  매번 `X-Kiosk-Key: <kiosk_identifier>:<raw_key>`를 보내야 한다. 조건부 요청도 동일하다.
  로그인 Bearer 토큰은 사용할 수 없다. Swagger의 `KioskApiKey`에도 같은 값을 입력한다.

```http
GET /api/v1/status-lists/1
X-Kiosk-Key: <kiosk_identifier>:<raw_key>
If-None-Match: W/"<이전에 받은 ETag>"
```

위 값은 형식 안내용 자리표시자다. 키는 안전하게 전달받아 기기에 보관하고 URL,
소스 코드, 로그에 넣지 않는다. 키 발급/회전 관리 API와 #42의 Bearer 형식 통일은
별도 작업이며, 이번 수정이 운영 키를 자동 생성하지 않는다.
...
401 본문은 `{"detail":{"code":"KIOSK_KEY_INVALID"}}` 등의 구조다.
...
- 현재 200/304 응답은 `Cache-Control: private, no-cache`, `Vary: X-Kiosk-Key`다.
...
- 최종 인증 헤더와 키 배포·회전 절차 및 인증 실패 시 기기 동작을 담당자와 확정한다.
