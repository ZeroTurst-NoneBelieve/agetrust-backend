...
단, 이 커밋은 **감사 로그 외에 롤백되어야 할 업무 변경이 세션에 없을
때만** 안전하다. 커밋은 세션 전체를 대상으로 하므로, 보류 중인 부분 변경이
있는 상태에서 위 문장을 그대로 따르면 그 변경까지 함께 저장된다. 실패로
끝나야 할 요청이 절반만 반영된 채 남는다.

현재 호출부는 두 경우 모두 조건을 만족한다.

- **로그인 실패**: 감사 로그 외에 세션에 담긴 변경이 없다.
- **OTP 실패**: `attempt_count` 증가가 함께 커밋되는데, 이 값은 롤백되면
  안 된다. 시도 횟수가 남지 않으면 상한 자체가 무의미해지기 때문이다.

새 실패 경로를 추가할 때는 이 조건부터 확인한다. 조건이 깨지는 자리라면
커밋 대신 별도 세션에서 감사 로그를 기록해야 한다.
"""
